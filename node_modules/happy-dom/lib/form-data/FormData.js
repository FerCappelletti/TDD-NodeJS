    ...
    constructor(form) {
        ...
        if (form) {
            for (const name of Object.keys(form.elements._namedItems)) {
                const radioNodeList = form.elements._namedItems[name];
                let element = radioNodeList[0];
                ...
                if (element.name && SUBMITTABLE_ELEMENTS.includes(element.tagName)) {
                    if (element.tagName === 'INPUT' && element.type === 'file') {
                        if (element.files.length === 0) {
                            this.append(element.name, new File_1.default([], '', { type: 'application/octet-stream' }));
                        }
                        else {
